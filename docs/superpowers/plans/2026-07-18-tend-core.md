...
> **Status:** this document guided the initial build (merged in PR #1) and was kept in sync through that point. Real usage after merge surfaced further bugs and design gaps that were fixed directly against the live code rather than back-ported here line by line — treat this file as the historical build record, not a live spec. `git log` and the current source are authoritative for anything past the initial merge.
...
def animate_squish(console, world, index, render_frame, cleared_count: int) -> None:
    ...
    # "Neighbors" means the two adjacent stalks only — never the whole
    # board. A thump is a gain event; the only whole-screen effect allowed
    # is loss.
...
def cmd_draw(state_path: Path | None = None) -> None:
    ...
    if key in "1234567":
        ...
        # Otherwise this stalk's stale this-tick bookkeeping re-triggers its own
        # just-resolved message on the redraw (e.g. "3 is being chewed. press 3."
        # right after pressing 3). died_this_tick/revived_this_tick are left
        # alone — those stay true regardless of what else gets squished.
