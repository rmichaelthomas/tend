...
**Interfaces:**
- Consumes: `tend.state.World`, `DEAD`, `DYING` (Task 1). Reads only per-tick fields already defined on `World` in Task 1 (`died_this_tick`, `revived_this_tick`, `seeds_spent_this_tick`, `spawned_indices_this_tick`, `chewed_this_tick`, `quiet_ticks`, `stalks[].height`, `stalks[].base`) — does not call into `engine.py`.
